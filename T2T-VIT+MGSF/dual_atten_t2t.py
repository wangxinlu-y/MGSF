...
class DualAttention(nn.Module):
    def __init__(self, dim, num_tokens, num_heads=12, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.hyper_token = nn.Sequential(
            nn.Linear(num_tokens * self.head_dim, 512),
            nn.LeakyReLU(),
            nn.Linear(512, 1),
            nn.Sigmoid()
        )
...
class T2TWithDualAttention(nn.Module):
    def __init__(self, t2t_model, num_heads=6):
        super().__init__()
        self.t2t = t2t_model
        embed_dim = t2t_model.embed_dim

        num_tokens = t2t_model.tokens_to_token.num_patches + 1
        self.dual_modules = nn.ModuleList([
            DualAttention(dim=embed_dim,
                          num_heads=num_heads,
                          num_tokens=num_tokens)
            for _ in range(len(t2t_model.blocks) - 1)
        ])
        self.dual_norms = nn.ModuleList([
            nn.LayerNorm(embed_dim)
            for _ in range(len(t2t_model.blocks) - 1)
    ...
    def _freeze_t2t(self):
        for name, param in self.t2t.named_parameters():
            if "head" not in name:
                param.requires_grad = False

    def forward_features(self, x):
        B = x.shape[0]
        x = self.t2t.tokens_to_token(x)
        cls_tokens = self.t2t.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        x = x + self.t2t.pos_embed
        x = self.t2t.pos_drop(x)
        for i, blk in enumerate(self.t2t.blocks):
            x = blk(x)
            if i < len(self.dual_modules):
                x = x + self.dual_modules[i](self.dual_norms[i](x))
        x = self.t2t.norm(x)
